...
void Widget::createWidgets () {
    ...
    mattab->setItem(0, 0, item);
    item = new QTableWidgetItem;
    item->setText("M");
    item->setBackground(QBrush("orange"));
    item->setFlags(Qt::ItemIsEditable);
    mattab->setItem(1, 1, item);
...
void Widget::slotButton() {
    if (sender() == but) {
        ...
        for (int i = 0; i < parts.size(); ++i) {
            QTableWidgetItem *item = new QTableWidgetItem;
            QString s;
            s = mattab->item(parts[i].first, parts[i].second)->text();
            item->setBackground(QBrush("green"));
            item->setText(s);
            mattab->setItem(parts[i].first, parts[i].second, item);
        }
        combo->setVisible(0);
        but->setVisible(0);
